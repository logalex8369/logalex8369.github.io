...
let selectedWord = (allWords[Math.floor(Math.random() * (allWords.length - 1))]).toLowerCase();
...
function check() {
  if (selectedWord.toLowerCase() == document.getElementById("myWord").value.toLowerCase()) {
    ...
    if (counter == 1){
      alert("Game Over. You guessed 1 word correctly");
    }else{
      alert("Game Over. You guessed " + counter + " words correctly");
    }
  }
}
